...
export async function getPassCode(req: Request, res: Response, next: NextFunction) {
  try {
    const data = await passCodeService.getPassCodeData();
...
export async function regeneratePassCode(req: Request, res: Response, next: NextFunction) {
  try {
    const { maxUses } = z.object({ maxUses: z.number().min(1).default(1) }).parse(req.body);
    ...
    const code = await passCodeService.regenerateCode(adminName, maxUses);
    const data = await passCodeService.getPassCodeData();
    const history = await passCodeService.getCodeHistory(5);
    logger.info(`ADMIN: PassCode regenerated by ${adminName} (Max: ${maxUses})`);
    res.json({ ...data, history });
  } catch (error) { next(error); }
}
...
export async function updateSetting(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    await logAdminAction(req.user!.userId, 'SETTING_UPDATE', { key, value });
