...
export default function UserDetailModal({ userId, onClose }: Props) {
  ...
  const [tab, setTab] = useState<'overview'|'transactions'|'mining'>('overview');
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="glass-card p-6 w-full max-w-2xl max-h-[85vh] overflow-y-auto relative z-10 animate-fade-up" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-1 mb-4">
          {(['transactions','mining'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === 'transactions' ? '💳 Transakce' : '⛏ Těžba'}
            </button>
          ))}
        </div>
        ...
        {tab === 'transactions' && (
          <div className="space-y-1.5 max-h-60 overflow-y-auto">
            ...
            )) : <p className="text-text-muted text-center py-4">Žádné transakce</p>}
            {data.transactions && data.transactions.length === 0 && <p className="text-text-muted text-center py-4">Žádné transakce</p>}
          </div>
        )}

        {tab === 'mining' && (
          <div className="space-y-1.5 max-h-60 overflow-y-auto">
            {data.mining ? data.mining.map((m: any) => (
            ...
            )) : <p className="text-text-muted text-center py-4">Žádná těžba</p>}
            {data.mining && data.mining.length === 0 && <p className="text-text-muted text-center py-4">Žádná těžba</p>}
          </div>
        )}
      </div>
    </div>
